...
bool checkWin (std::vector<std::vector<std::string>> gameBoard) {
  ...
  for (int i = 0; i < gameBoard.size(); i++) {
    if (gameBoard[i][0] == player && gameBoard[i][1] == player && gameBoard[i][2] == player) {
      if (player == X) {
        std::cout << "Game Over! Player 1 (" << playerOneName << ") wins! \n\n";
        return true;
      } else {
        std::cout << "Game Over! Player 2 (" << playerTwoName << ") wins! \n\n";
        return true;
      }
    }
  }
  ...
  if (player == X) {
      std::cout << "Game Over! Player 1 (" << playerOneName << ") (X) wins! \n\n";
      return true;
    } else {
      std::cout << "Game Over! Player 2 (" << playerTwoName << ") (O) wins! \n\n";
      return true;
    }
  }
  return false;
